...
TEST_F(LookupLolaProxyElementFixture, ReturnsValidResultForEventLookup)
{
    ...
    // Then the result is valid and contains the expected element FQ ID
...
TEST_F(LookupLolaProxyElementFixture, ReturnsValidResultForFieldLookup)
{
    ...
    // Then the result is valid with FIELD element type
    ASSERT_TRUE(result.has_value());
...
TEST_F(LookupLolaProxyElementFixture, ReturnsValidResultForMethodLookup)
{
    ...
    // Then the result is valid with METHOD element type
    ASSERT_TRUE(result.has_value());
...
{
    // Given a proxy base with a null binding (non-lola)
    const auto handle = kConfigStoreAsilB.GetHandle();
...
{
    // Given a handle with SomeIP deployment (non-lola)
...
{
    // Given a handle with blank deployment (non-lola)
